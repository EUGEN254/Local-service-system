...
const SpChatSidebar = ({ selectedUser, setSelectedUser }) => {
  const { backendUrl, socket } = useContext(ShareContext);
  ...
  useEffect(() => {
    ...
    const handleOnlineUsers = (users) => setOnlineUsers(users.map((id) => id.toString()));
  ...
  if (loading)
    return <div className="flex items-center justify-center h-full text-gray-500">Loading customers...</div>;

  if (customers.length === 0)
    return <div className="flex items-center justify-center h-full text-gray-500">No active customers yet</div>;

  return (
    <div className={`bg-white h-full border-r border-gray-200 p-5 overflow-y-auto rounded-tl-2xl rounded-bl-2xl transition-all duration-300 
