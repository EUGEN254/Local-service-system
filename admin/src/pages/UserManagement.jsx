...
// ---------------------------------------------------------------------------
// The primary admin email must be exposed to the frontend via Vite/CRA env.
// Add this to your .env file:
//   VITE_PRIMARY_ADMIN_EMAIL=bitinyoeugen@gmail.com   (Vite)
//   REACT_APP_PRIMARY_ADMIN_EMAIL=bitinyoeugen@gmail.com  (CRA)
// ---------------------------------------------------------------------------
const PRIMARY_ADMIN_EMAIL =
  import.meta.env?.VITE_PRIMARY_ADMIN_EMAIL ||
  process.env?.REACT_APP_PRIMARY_ADMIN_EMAIL ||
  "";

const isPrimaryAdmin = (user) =>
  PRIMARY_ADMIN_EMAIL && user.email === PRIMARY_ADMIN_EMAIL;
...
const UserManagement = () => {
  ...
  const handleEditSubmit = async (e) => {
    ...
    // Extra client-side guard: do not allow demoting the primary admin
    if (isPrimaryAdmin(selectedUser) && editForm.role !== "admin") {
      toast.error("The primary admin's role cannot be changed.");
      return;
    }
  ...
  const handleStatusToggle = async (user) => {
    // Primary admin status should not be toggled either (optional – remove if not needed)
    if (isPrimaryAdmin(user)) {
      toast.error("The primary admin's status cannot be changed.");
      return;
    }
  ...
  return (
    <div className="p-6">
          ...
          <table className="w-full min-w-[800px]">
            ...
            <tbody className="divide-y divide-gray-200">
              {currentUsers.map((user, idx) => {
                const locked = isPrimaryAdmin(user);
                return (
      ...
      {/* ── Add User Modal ──────────────────────────────────────────────────── */}
      {showAddModal && (
        <div className="fixed inset-0 bg-black/70 bg-opacity-100 flex items-center justify-center z-50 p-4">
          ...
          <div className="bg-white rounded-lg max-w-md w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              ...
              {isPrimaryAdmin(selectedUser) && (
                <div className="mb-4 flex items-center gap-2 px-3 py-2 bg-yellow-50 border border-yellow-200 rounded-lg text-yellow-700 text-sm">
                  <FaLock className="shrink-0" />
                  This is the primary admin. Name, phone and status can be
                  edited, but the role is locked to <strong>Admin</strong>.
                </div>
              )}
              ...
              <form onSubmit={handleEditSubmit} className="space-y-4">
                ...
                <div>
                  ...
                  {isPrimaryAdmin(selectedUser) ? (
                    /* Locked role display for primary admin */
                    <div className="flex items-center gap-2 w-full px-3 py-2 border border-gray-200 bg-gray-50 rounded-lg text-gray-500">
                      <FaLock className="text-xs" />
                      Admin (locked)
                    </div>
                  ) : (
